...
- [Lists](#lists)
  ...
  - [Sorting](#sorting)
  - [Updating lists (reconciliation)](#updating-lists-reconciliation)
...
### Updating lists (reconciliation)

When you reassign `items` on a list that is already rendered, M2D2 no longer
clears the container and rebuilds every node. Instead it **reconciles**: it
reuses the item nodes that are already there, updates their content in place,
and only adds or removes the nodes that are needed.
...
// The same <li> elements are reused, only their text is updated:
list.items = [{ name: "A2" }, { name: "B2" }];
```

Why it matters:

- **Focus, selection and scroll are preserved** — a focused `<input>` inside an
  item keeps focus when the list updates, because the element is reused rather
  than replaced.
- **Less DOM churn** — only what actually changed is touched, which is faster
  for large lists.
- **State survives** — CSS transitions, animations and any per-item state stay
  attached to the reused nodes.

Notes:

- `dataset.id` still reflects the **positional index** of each item, so
  `items.get(i)` and `index` keep working as before.
- Assigning `items` (and the items methods like `push`, `splice`, `sort`, …)
  fires the `update` event **exactly once** per change.
- Reconciliation patches each reused item with the new data, so it works best
  with templates whose structure is fixed. If an item's internal structure
  changes between renders, declare a `key` (below).

### Keyed lists

By default M2D2 reconciles **by position** (item `i` reuses node `i`). That is
ideal when you replace the whole list with data of the same shape. But when the
order changes — sorting, filtering, prepending — positional reuse matches the
wrong node to each piece of data. To reuse nodes **by identity** instead,
declare a `key`:

```js
const list = $("#list", {
    key: "id",                                    // a field of each item
    template: { li: { span: { css: "name" } } },
    items: [{ id: 1, name: "A" }, { id: 2, name: "B" }, { id: 3, name: "C" }]
});

// Reverse the order: each <li> is reused and moved, not rebuilt:
list.items = [{ id: 3, name: "C" }, { id: 2, name: "B" }, { id: 1, name: "A" }];
```

`key` accepts:

- a **field name** (string) — the value of that field identifies the item;
- a **function** — `key: (item, index) => …` returns the identifier, useful for
  computed keys.

```js
$("#list", { key: (item) => item.uuid, template: { … }, … });
```

Good to know:

- `key` is **optional**. Leave it out and M2D2 reconciles by position — no
  change is required for existing lists.
- `dataset.id` **stays the positional index** even in keyed mode; the key is
  used only for matching.
- When `key` is a field name, that field is **consumed** for matching and is not
  rendered into the item, so you can key on any identifier (`id`, `uuid`,
  `sku`, …) without it producing an "unknown key" warning or appearing on the
  element. (With a function key, the fields it reads still follow the normal
  rules.)
