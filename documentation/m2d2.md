...
users.items.selected().next().selected = true // select the next item from the selected one
users.items.selected().prev().selected = true // select the previous item from the selected one
```
...
nodes, types, properties, attributes, etc. This is the list of functions you can use:
...
$.prependAllChild
```
For details on each function, take a look to [the source code](../js/utils.src.js).
